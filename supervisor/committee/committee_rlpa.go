...
func NewRLPACommitteeModule(Ip_nodeTable map[uint64]map[uint64]string, Ss *signal.StopSignal, sl *supervisor_log.SupervisorLog, csvFilePath string, dataNum, batchNum, rlpaFrequency int) *RLPACommitteeModule {
	...
	cg.Init_RLPAState(0.5, 100, params.ShardNum)
...
// func (ccm *RLPACommitteeModule) fetchModifiedMapC(key string) uint64 {
// 	if val, ok := ccm.modifiedMap2[key]; !ok {
// 		return uint64(utils.Addr2Shard(key))
// 	} else {
// 		return val
// 	}
...
func (ccm *RLPACommitteeModule) HandleBlockInfo(b *message.BlockInfoMsg) {
	...
	// 	// t_now := time.Now().Unix()
	// 	// ccm.rlpaGraph.AddEdgeWithTime(partition.Vertex{Addr: tx.Sender}, partition.Vertex{Addr: tx.Recipient}, t_now, TimeWindow)
	// 	ccm.rlpaGraph.AddEdge(partition.Vertex{Addr: tx.Sender}, partition.Vertex{Addr: tx.Recipient}, 0)
	// }
	// for _, r2tx := range b.Relay2Txs {
	// 	t_now := time.Now().Unix()
	// 	ccm.rlpaGraph.AddEdgeWithTime(partition.Vertex{Addr: r2tx.Sender}, partition.Vertex{Addr: r2tx.Recipient}, t_now, TimeWindow)
	// 	// ccm.rlpaGraph.AddEdge(partition.Vertex{Addr: r2tx.Sender}, partition.Vertex{Addr: r2tx.Recipient}, 1)
	// }
	...
	for _, tx := range b.InnerShardTxs {
		...
		// 检查是否为热点账户
		if ccm.rlpaGraph.IsHotAccount(tx.Sender) || ccm.rlpaGraph.IsHotAccount(tx.Recipient) {
			ccm.rlpaGraph.AddEdgeWithTime(partition.Vertex{Addr: tx.Sender}, partition.Vertex{Addr: tx.Recipient}, tx.Time.Unix(), TimeWindow)
		}
		// ccm.rlpaGraph.AddEdge(partition.Vertex{Addr: tx.Sender}, partition.Vertex{Addr: tx.Recipient}, 0)
	}
	for _, r2tx := range b.Relay2Txs {
		...
		if ccm.rlpaGraph.IsHotAccount(r2tx.Sender) || ccm.rlpaGraph.IsHotAccount(r2tx.Recipient) {
			ccm.rlpaGraph.AddEdgeWithTime(partition.Vertex{Addr: r2tx.Sender}, partition.Vertex{Addr: r2tx.Recipient}, r2tx.Time.Unix(), TimeWindow)
		}
	}
	ccm.clpaLock.Unlock()
	ccm.rlpaLock.Unlock()
