...
type RGraph struct {
	...
	EdgeTimes map[Vertex]map[Vertex][]int64 // 记录每条边的交易时间
}
...
// 增加图中的边，动态计算关联度
func (g *RGraph) AddEdgeWithTime(u, v Vertex, t_now int64, T int64) {
	if g.EdgeSet == nil {
		g.EdgeSet = make(map[Vertex]map[Vertex]float64)
	}
	if g.EdgeTimes == nil {
		g.EdgeTimes = make(map[Vertex]map[Vertex][]int64)
	}
	if g.EdgeSet[u] == nil {
		g.EdgeSet[u] = make(map[Vertex]float64)
	}
	...
	if g.EdgeTimes[u] == nil {
		g.EdgeTimes[u] = make(map[Vertex][]int64)
	}
	if g.EdgeTimes[v] == nil {
		g.EdgeTimes[v] = make(map[Vertex][]int64)
	}
...
// 计算关联度
func (g *RGraph) calculateAssociation(times []int64, t_now int64, T int64) float64 {
	association := 0.0
	for _, t_i := range times {
		if t_now-t_i <= T { // 仅考虑时间窗口 T 内的交易
			association += math.Exp(-0.1 * float64(t_now-t_i))
		}
	}
	return association / float64(T)
}
