...
// 更新分片性能值
func (pl *PLouvainState) UpdateShardPerformance(newShardPerformance []float32) {
	pl.shardPerformance = newShardPerformance
}

// 更新数据
func (pl *PLouvainState) UpdateData(newIptable map[uint64]map[uint64]string, newShardPerformance []float32) {
	pl.shardNum = len(newIptable)
	for shardId, nodeTable := range newIptable {
		pl.VertexsNumInShard[shardId] = len(nodeTable)
	}
	pl.shardPerformance = newShardPerformance
}
...
func (pl *PLouvainState) PLouvain_Partition() map[string]uint64 {
	...
	// var shardNum = 4
	// shardPerformance := []float32{1000, 800, 800, 600}
	// var israndom bool = false
	// var beta float32 = 2.0
